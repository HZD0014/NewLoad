...
class Dataset_Load(Dataset):
    def __init__(self, root_path='./dataset', flag='train', size=None,
                 features='MS', data_path='PanamaLoad.csv',
                 target='LOAD', scale=True, timeenc=0, freq='h'):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4   # 输入长度 96
            self.label_len = 24
            self.pred_len = 24      # 预测长度 24
        else:
            self.seq_len = size[0]
            self.label_len = size[1]
            self.pred_len = size[2]
    ...
    def __read_data__(self):
        ...
        '''
        # 创建 24 个新的列，分别存放前 24 个时间步的目标变量
        for i in range(1, 25):
            df_raw[f'history_{i}'] = df_raw[self.target].shift(i)

        # 删除包含 NaN 值的行，因为前 24 个时间步无法有完整的滞后数据
        ...
        if self.scale:
            ...
            data = df_raw.drop(columns=['date']).values
